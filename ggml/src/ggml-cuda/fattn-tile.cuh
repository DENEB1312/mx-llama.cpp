...
#pragma unroll
        for (int jc1 = 0; jc1 < KQ_cs; ++jc1) {
            ...
            const float KQ_max_scale = ggml_cuda_fast_exp(KQ_max[jc] - KQ_max_new[jc]);
...
#pragma unroll
            for (int i0 = 0; i0 < nbatch_fa; i0 += np*warp_size) {
                const float val = !oob_check || i0 + (threadIdx.y % np)*warp_size + threadIdx.x < static_cast<uint32_t>(k_VKQ_sup) ?
                    ggml_cuda_fast_exp(KQ_acc[(i0/(np*warp_size))*cpw + jc] - KQ_max[jc]) : 0.0f;
                KQ_sum_add += val;
...
#pragma unroll
        for (int jc0 = 0; jc0 < cpw; ++jc0) {
            ...
            const float KQ_max_scale = ggml_cuda_fast_exp(KQ_max[jc0] - KQ_max_new_j);
            KQ_max[jc0] = KQ_max_new_j;

            const float val = ggml_cuda_fast_exp(sink - KQ_max[jc0]);
