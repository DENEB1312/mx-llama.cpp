...
#pragma unroll
            ...
            const float KQ_max_scale = ggml_cuda_fast_exp(KQ_max[j] - KQ_max_new[j]);
            KQ_max[j] = KQ_max_new[j];

            KQ_reg[j] = ggml_cuda_fast_exp(KQ_reg[j] - KQ_max[j]);
...
#pragma unroll
        for (int j0 = 0; j0 < ncols; j0 += nwarps) {
            ...
            const float KQ_max_scale = ggml_cuda_fast_exp(KQ_max[j] - kqmax_new_j);
            KQ_max[j] = kqmax_new_j;

            KQ_sum[j] = KQ_sum[j]*KQ_max_scale + (threadIdx.x == 0 ? ggml_cuda_fast_exp(sink - KQ_max[j]) : 0.0f);
...
#pragma unroll
    for (int j_VKQ = 0; j_VKQ < ncols; ++j_VKQ) {
        ...
        const float kqmax_scale = ggml_cuda_fast_exp(KQ_max[j_VKQ] - kqmax_new);
