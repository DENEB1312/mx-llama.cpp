...
GFX906_ENV = {
    ...
    # graph path. With graphs on, rocprof may collapse launches into replayed graph nodes.
...
def short_name(name: str) -> str:
    # llama.cpp custom HIP kernels: "void mul_mat_vec_q<(ggml_type)8, ...>" -> "mmvq_Q8"
    m = re.search(r'(mul_mat_vec_q|mul_mat_q|mul_mat_id|mmvq|mmq|mul_mat)\b', name)
    if m:
        base = m.group(1)
        t = re.search(r'ggml_type\)(\d+)', name)
        return f"{base}<Q{t.group(1)}>" if t else base
    ...
    # Strip the "void " prefix and template noise for anything else.
    s = name.split('<')[0].replace('void ', '').strip()
    return s if s else name[:50]
